    ...
    return (
        ...
        <motion.div  whileHover={{scale: 1.1, cursor: 'pointer'}} transition={{duration: .5}} key={post.slug} className="flex flex-col w-full bg-white rounded-lg lg:max-w-lg xl:lg:max-w-xl ">
            ...
            <div className="p-5 flex flex-col ">
                <p className="h-max-20 h-16 font-semibold text-lg prose prose-slate">{post.title}</p>
                <p className="h-max-20 h-8 text-gray-500 prose prose-slate">Post on <span className="text-blue-500">{timePublish}</span> by <span className="text-blue-500">{post.author.name}</span></p>
                <p className="text-justify tracking-tight h-max-40 prose prose-slate">{post.excerpt}</p>
                <div className="md:flex md:justify-between md:items-center pt-5 prose prose-slate">
